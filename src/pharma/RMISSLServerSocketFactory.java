...
public class RMISSLServerSocketFactory implements RMIServerSocketFactory {
	...
	un tipo di autenticazione denominata unilaterale : solo il server è autenticato
	(il client conosce l’identità del server), ma non vice-versa (il client rimane 
	...
	L’implementazione di default del Java Keystore è basata su file, 
	quest’ultimo è in un formato proprietario denominato JKS. 
	...
	Il nome solitamente attribuito a questo repository è keystore;
	- Client side: Java Keystore che contiene i soli certificati utilizzati dalle
 	applicazioni che agiscono come client. 
 	Il nome solitamente attribuito a questo repository è trustStore.
 	La locazione predefinita di entrambi i keystore è <java-home>/lib/security/
	*/
	...
	public ServerSocket createServerSocket(int port) throws IOException{
		...
		try{
			...
			KeyManagerFactory keymng = KeyManagerFactory.getInstance("SunX509"); //e' il default key manager alg per le sun jvm
